$(document).ready(function () {
    $('#PackagingInnovationOurOfferingForm').on('submit', function (e) {
        e.preventDefault();
       // Clear previous error messages
        $('#error_title, #error_description, #error_image').text('');
        var formData = new FormData(this);
        $.ajax({
            url: frontend + "admin/save_packaging_innovation_our_offering",  // Adjust URL accordingly
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
            ...
            success: function (response) {
                if (response.status === 'success') {
                    ...
                    $('#PackagingInnovationOurOfferingForm')[0].reset();
                     // Reload the DataTable
                    PackagingInnovationOurOfferingTable.ajax.reload(null, false);
...
$(document).ready(function () {
    ...
    PackagingInnovationOurOfferingTable = $('#PackagingInnovationOurOfferingTable').DataTable({
        ajax: {
            url: frontend + "admin/get_packaging_innovation_our_offering_data",  // Adjust URL accordingly
            type: 'POST',
    ...
    $("#PackagingInnovationOurOfferingTable").on("click", ".view-btn", function (e) {
        e.preventDefault();
        const id = $(this).data("id");
        $.ajax({
            url: frontend + "admin/get_packaging_innovation_our_offering_details",
            type: "POST",
            dataType: "json",
    ...
    $("#PackagingInnovationOurOfferingTable").on("click", ".edit-btn", function (e) {
        e.preventDefault();
        const id = $(this).data("id");
    ...
    // Delete action
	$("#PackagingInnovationOurOfferingTable").on("click", ".delete-btn", function (e) {
		e.preventDefault();
		const id = $(this).data("id");
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				$.ajax({
					url: frontend + "admin/delete_packaging_innovation_our_offering",
					type: "POST",
					data: { id: id },
					dataType: "json",
					success: function (response) {
...
$('#EditPackagingInnovationOurOfferingForm').submit(function (e) {
    e.preventDefault();
    ...
    $.ajax({
        url: frontend + "admin/update_packaging_innovation_our_offering", // adjust to your route
        type: "POST",
        data: formData,
        dataType: "json",
        contentType: false,
        ...
        success: function (response) {
            if (response.status === 'success') {
                ...
                // Reset the form
                $('#EditPackagingInnovationOurOfferingForm')[0].reset();
