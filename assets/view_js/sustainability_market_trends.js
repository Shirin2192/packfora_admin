$(document).ready(function () {
    $('#SustainabilityMarketTrendsForm').on('submit', function (e) {
        e.preventDefault();
        // Clear previous error messages
        $('#error_title, #error_description').text('');
        var formData = new FormData(this);
        $.ajax({
            url: frontend + "admin/save_sustainability_market_trends",  // Adjust URL accordingly
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
            ...
            success: function (response) {
                if (response.status === 'success') {
                    ...
                    $('#SustainabilityMarketTrendsForm')[0].reset();
                     // Reload the DataTable
                    SustainabilityMarketTrendsTable.ajax.reload(null, false);
...
$(document).ready(function () {
    ...
    SustainabilityMarketTrendsTable = $('#SustainabilityMarketTrendsTable').DataTable({
        ajax: {
            url: frontend + "admin/get_sustainability_market_trends_data",  // Adjust URL accordingly
            type: 'POST',
    ...
    $("#SustainabilityMarketTrendsTable").on("click", ".view-btn", function (e) {
        e.preventDefault();
        const id = $(this).data("id");
        $.ajax({
            url: frontend + "admin/get_sustainability_market_trends_details",
            type: "POST",
            dataType: "json",
    ...
    $("#SustainabilityMarketTrendsTable").on("click", ".edit-btn", function (e) {
        e.preventDefault();
        const id = $(this).data("id");
    ...
    // Delete action
	$("#SustainabilityMarketTrendsTable").on("click", ".delete-btn", function (e) {
		e.preventDefault();
		const id = $(this).data("id");
		Swal.fire({
			title: "Are you sure?",
			...
			if (result.isConfirmed) {
				$.ajax({
					url: frontend + "admin/delete_sustainability_market_trends",
					type: "POST",
					data: { id: id },
					dataType: "json",
					success: function (response) {
...
$('#EditSustainabilityMarketTrendsForm').submit(function (e) {
    e.preventDefault();
    let formData = new FormData(this);
    // Clear previous errors
    $('#error_edit_title, #error_edit_description').text('');
    $.ajax({
        url: frontend + "admin/update_sustainability_market_trends_details", // adjust to your route
        type: "POST",
        data: formData,
        dataType: "json",
        contentType: false,
        ...
        success: function (response) {
            if (response.status === 'success') {
                ...
                // Reset the form
                $('#EditSustainabilityMarketTrendsForm')[0].reset();
