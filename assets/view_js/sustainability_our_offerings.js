$(document).ready(function () {
    $('#SustainabilityOurOfferingForm').on('submit', function (e) {
        e.preventDefault();
       // Clear previous error messages
        $('#error_title, #error_description, #error_image').text('');
        var formData = new FormData(this);
        $.ajax({
            url: frontend + "admin/save_sustainability_our_offering",  // Adjust URL accordingly
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
            ...
            success: function (response) {
                if (response.status === 'success') {
                    ...
                    $('#SustainabilityOurOfferingForm')[0].reset();
                     // Reload the DataTable
                    SustainabilityOurOfferingTable.ajax.reload(null, false);
...
$(document).ready(function () {
    ...
    SustainabilityOurOfferingTable = $('#SustainabilityOurOfferingTable').DataTable({
        ajax: {
            url: frontend + "admin/get_sustainability_our_offering_data",  // Adjust URL accordingly
            type: 'POST',
    ...
    $("#SustainabilityOurOfferingTable").on("click", ".view-btn", function (e) {
        e.preventDefault();
        const id = $(this).data("id");
        $.ajax({
            url: frontend + "admin/get_sustainability_our_offering_details",
            type: "POST",
            dataType: "json",
    ...
    $("#SustainabilityOurOfferingTable").on("click", ".edit-btn", function (e) {
        e.preventDefault();
        const id = $(this).data("id");
    ...
    // Delete action
	$("#SustainabilityOurOfferingTable").on("click", ".delete-btn", function (e) {
		e.preventDefault();
		const id = $(this).data("id");
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				$.ajax({
					url: frontend + "admin/delete_sustainability_our_offering",
					type: "POST",
					data: { id: id },
					dataType: "json",
					success: function (response) {
...
$('#EditSustainabilityOurOfferingForm').submit(function (e) {
    e.preventDefault();
    ...
    $.ajax({
        url: frontend + "admin/update_sustainability_our_offering", // adjust to your route
        type: "POST",
        data: formData,
        dataType: "json",
        contentType: false,
        ...
        success: function (response) {
            if (response.status === 'success') {
                ...
                // Reset the form
                $('#EditSustainabilityOurOfferingForm')[0].reset();
