$(document).ready(function () {
    $('#ShineWithUsForm').on('submit', function (e) {
        e.preventDefault();
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.status === 'success') {
                    ...
                    $('#ShineWithUsForm')[0].reset();
                     // Reload the DataTable
                    ShineWithUsTable.ajax.reload(null, false);
...
$(document).ready(function () {
    ...
    $("#ShineWithUsTable").on("click", ".view-btn", function (e) {
        e.preventDefault();
        const id = $(this).data("id");

        $.ajax({
            url: frontend + "admin/get_shine_with_us_details",
            type: "POST",
            dataType: "json",
    ...
    $("#ShineWithUsTable").on("click", ".edit-btn", function (e) {
        e.preventDefault();
        const id = $(this).data("id");
    ...
    // Delete action
	$("#ShineWithUsTable").on("click", ".delete-btn", function (e) {
		e.preventDefault();
		const id = $(this).data("id");
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				$.ajax({
					url: frontend + "admin/delete_shine_with_us",
					type: "POST",
					data: { id: id },
					dataType: "json",
					success: function (response) {
...
$('#EditShineWithUsForm').submit(function (e) {
    e.preventDefault();
    ...
    $.ajax({
        url: frontend + "admin/update_shine_with_us", // adjust to your route
        type: "POST",
        data: formData,
        dataType: "json",
        contentType: false,
