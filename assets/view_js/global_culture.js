$(document).ready(function () {
    $('#GlobalCultureForm').on('submit', function (e) {
        e.preventDefault();
        ...
        $.ajax({
            url: frontend + "admin/save_global_culture",  // Adjust URL accordingly
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
            ...
            success: function (response) {
                if (response.status === 'success') {
                    ...
                    $('#GlobalCultureForm')[0].reset();
                     // Reload the DataTable
                    GlobalCultureTable.ajax.reload(null, false);
...
$(document).ready(function () {
    ...
    GlobalCultureTable = $('#GlobalCultureTable').DataTable({
        ajax: {
            url: frontend + "admin/get_global_culture_data",  // Adjust URL accordingly
            type: 'POST',
    ...
    $("#GlobalCultureTable").on("click", ".view-btn", function (e) {
        e.preventDefault();
        const id = $(this).data("id");

        $.ajax({
            url: frontend + "admin/get_global_culture_details",
            type: "POST",
            dataType: "json",
    ...
    $("#GlobalCultureTable").on("click", ".edit-btn", function (e) {
        e.preventDefault();
        const id = $(this).data("id");
    ...
    // Delete action
	$("#GlobalCultureTable").on("click", ".delete-btn", function (e) {
		e.preventDefault();
		const id = $(this).data("id");
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				$.ajax({
					url: frontend + "admin/delete_global_culture",
					type: "POST",
					data: { id: id },
					dataType: "json",
					success: function (response) {
...
$('#EditGlobalCultureForm').submit(function (e) {
    e.preventDefault();
    ...
    $.ajax({
        url: frontend + "admin/update_global_culture", // adjust to your route
        type: "POST",
        data: formData,
        dataType: "json",
        contentType: false,
