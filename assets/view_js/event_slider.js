$(document).ready(function () {
    $('#EventSliderForm').on('submit', function (e) {
        e.preventDefault();
        ...
        $.ajax({
            url: frontend + "admin/save_event_slider",  // Adjust URL accordingly
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
            ...
            success: function (response) {
                if (response.status === 'success') {
                    ...
                    $('#EventSliderForm')[0].reset();
                     // Reload the DataTable
                    EventSliderTable.ajax.reload(null, false);
...
$(document).ready(function () {
    ...
    EventSliderTable = $('#EventSliderTable').DataTable({
        ajax: {
            url: frontend + "admin/get_event_slider_data",  // Adjust URL accordingly
            type: 'POST',
    ...
    $("#EventSliderTable").on("click", ".view-btn", function (e) {
        e.preventDefault();
        const id = $(this).data("id");

        $.ajax({
            url: frontend + "admin/get_event_slider_details",
            type: "POST",
            dataType: "json",
    ...
    $("#EventSliderTable").on("click", ".edit-btn", function (e) {
        e.preventDefault();
        const id = $(this).data("id");
    ...
    // Delete action
	$("#EventSliderTable").on("click", ".delete-btn", function (e) {
		e.preventDefault();
		const id = $(this).data("id");
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				$.ajax({
					url: frontend + "admin/delete_event_slider",
					type: "POST",
					data: { id: id },
					dataType: "json",
					success: function (response) {
...
$('#EditEventSliderForm').submit(function (e) {
    e.preventDefault();
    ...
    $.ajax({
        url: frontend + "admin/update_event_slider", // adjust to your route
        type: "POST",
        data: formData,
        dataType: "json",
        contentType: false,
