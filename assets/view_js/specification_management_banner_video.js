$(document).ready(function () {
    $('#SpecificationManagementVideoBannerForm').on('submit', function (e) {
        e.preventDefault();
        // Clear previous error messages
        $('#error_video').text('');
        var formData = new FormData(this);
        $.ajax({
            url: frontend + "admin/save_specification_management_banner_video",  // Adjust URL accordingly
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
            ...
            success: function (response) {
                if (response.status === 'success') {
                    ...
                    $('#SpecificationManagementVideoBannerForm')[0].reset();
                     // Reload the DataTable
                    SpecificationManagementVideoBannerTable.ajax.reload(null, false);
...
$(document).ready(function () {
    ...
    SpecificationManagementVideoBannerTable = $('#SpecificationManagementVideoBannerTable').DataTable({
        ajax: {
            url: frontend + "admin/get_specification_management_banner_video_data",  // Adjust URL accordingly
            type: 'POST',
    ...
    $("#SpecificationManagementVideoBannerTable").on("click", ".view-btn", function (e) {
        e.preventDefault();
        const id = $(this).data("id");

        $.ajax({
            url: frontend + "admin/get_specification_management_banner_video_details",
            type: "POST",
            dataType: "json",
    ...
    $("#SpecificationManagementVideoBannerTable").on("click", ".edit-btn", function (e) {
        e.preventDefault();
        const id = $(this).data("id");
    ...
    // Delete action
	$("#SpecificationManagementVideoBannerTable").on("click", ".delete-btn", function (e) {
		e.preventDefault();
		const id = $(this).data("id");
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				$.ajax({
					url: frontend + "admin/delete_specification_management_banner_video",
					type: "POST",
					data: { id: id },
					dataType: "json",
					success: function (response) {
...
$('#EditSpecificationManagementVideoBannerForm').submit(function (e) {
    e.preventDefault();
    ...
    $.ajax({
        url: frontend + "admin/update_specification_management_banner_video", // adjust to your route
        type: "POST",
        data: formData,
        dataType: "json",
        contentType: false,
        ...
        success: function (response) {
            if (response.status === 'success') {
                ...
                // Reset the form
                $('#EditSpecificationManagementVideoBannerForm')[0].reset();
