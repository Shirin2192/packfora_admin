$(document).ready(function () {
    $('#TalentFlexResourcingModelForm').on('submit', function (e) {
        e.preventDefault();
       // Clear previous error messages
        $('#error_title, #error_description, #error_image').text('');
        var formData = new FormData(this);
        $.ajax({
            url: frontend + "admin/save_talent_flex_resourcing_model",  // Adjust URL accordingly
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
            ...
            success: function (response) {
                if (response.status === 'success') {
                    ...
                    $('#TalentFlexResourcingModelForm')[0].reset();
                     // Reload the DataTable
                    TalentFlexResourcingModelTable.ajax.reload(null, false);
...
$(document).ready(function () {
    ...
    TalentFlexResourcingModelTable = $('#TalentFlexResourcingModelTable').DataTable({
        ajax: {
            url: frontend + "admin/get_talent_flex_resourcing_model_data",  // Adjust URL accordingly
            type: 'POST',
    ...
    $("#TalentFlexResourcingModelTable").on("click", ".view-btn", function (e) {
        e.preventDefault();
        const id = $(this).data("id");
        $.ajax({
            url: frontend + "admin/get_talent_flex_resourcing_model_details",
            type: "POST",
            dataType: "json",
    ...
    $("#TalentFlexResourcingModelTable").on("click", ".edit-btn", function (e) {
        e.preventDefault();
        const id = $(this).data("id");
    ...
    // Delete action
	$("#TalentFlexResourcingModelTable").on("click", ".delete-btn", function (e) {
		e.preventDefault();
		const id = $(this).data("id");
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				$.ajax({
					url: frontend + "admin/delete_talent_flex_resourcing_model",
					type: "POST",
					data: { id: id },
					dataType: "json",
					success: function (response) {
...
$('#EditTalentFlexResourcingModelForm').submit(function (e) {
    e.preventDefault();
    ...
    $.ajax({
        url: frontend + "admin/update_talent_flex_resourcing_model", // adjust to your route
        type: "POST",
        data: formData,
        dataType: "json",
        contentType: false,
        ...
        success: function (response) {
            if (response.status === 'success') {
                ...
                // Reset the form
                $('#EditTalentFlexResourcingModelForm')[0].reset();
