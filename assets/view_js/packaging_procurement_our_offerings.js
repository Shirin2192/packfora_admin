$(document).ready(function () {
    $('#PackagingProcurementOurOfferingForm').on('submit', function (e) {
        e.preventDefault();
       // Clear previous error messages
        $('#error_title, #error_description, #error_image').text('');
        var formData = new FormData(this);
        $.ajax({
            url: frontend + "admin/save_packaging_procurement_our_offering",  // Adjust URL accordingly
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
            ...
            success: function (response) {
                if (response.status === 'success') {
                    ...
                    $('#PackagingProcurementOurOfferingForm')[0].reset();
                     // Reload the DataTable
                    PackagingProcurementOurOfferingTable.ajax.reload(null, false);
...
$(document).ready(function () {
    ...
    PackagingProcurementOurOfferingTable = $('#PackagingProcurementOurOfferingTable').DataTable({
        ajax: {
            url: frontend + "admin/get_packaging_procurement_our_offering_data",  // Adjust URL accordingly
            type: 'POST',
    ...
    $("#PackagingProcurementOurOfferingTable").on("click", ".view-btn", function (e) {
        e.preventDefault();
        const id = $(this).data("id");
        $.ajax({
            url: frontend + "admin/get_packaging_procurement_our_offering_details",
            type: "POST",
            dataType: "json",
    ...
    $("#PackagingProcurementOurOfferingTable").on("click", ".edit-btn", function (e) {
        e.preventDefault();
        const id = $(this).data("id");
    ...
    // Delete action
	$("#PackagingProcurementOurOfferingTable").on("click", ".delete-btn", function (e) {
		e.preventDefault();
		const id = $(this).data("id");
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				$.ajax({
					url: frontend + "admin/delete_packaging_procurement_our_offering",
					type: "POST",
					data: { id: id },
					dataType: "json",
					success: function (response) {
...
$('#EditPackagingProcurementOurOfferingForm').submit(function (e) {
    e.preventDefault();
    ...
    $.ajax({
        url: frontend + "admin/update_packaging_procurement_our_offering", // adjust to your route
        type: "POST",
        data: formData,
        dataType: "json",
        contentType: false,
        ...
        success: function (response) {
            if (response.status === 'success') {
                ...
                // Reset the form
                $('#EditPackagingProcurementOurOfferingForm')[0].reset();
