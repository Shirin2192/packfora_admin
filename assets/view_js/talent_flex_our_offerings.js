$(document).ready(function () {
    $('#TalentFlexOurOfferingForm').on('submit', function (e) {
        e.preventDefault();
       // Clear previous error messages
        $('#error_title, #error_description, #error_image').text('');
        var formData = new FormData(this);
        $.ajax({
            url: frontend + "admin/save_talent_flex_our_offering",  // Adjust URL accordingly
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
            ...
            success: function (response) {
                if (response.status === 'success') {
                    ...
                    $('#TalentFlexOurOfferingForm')[0].reset();
                     // Reload the DataTable
                    TalentFlexOurOfferingTable.ajax.reload(null, false);
...
$(document).ready(function () {
    ...
    TalentFlexOurOfferingTable = $('#TalentFlexOurOfferingTable').DataTable({
        ajax: {
            url: frontend + "admin/get_talent_flex_our_offerings_data",  // Adjust URL accordingly
            type: 'POST',
    ...
    $("#TalentFlexOurOfferingTable").on("click", ".view-btn", function (e) {
        e.preventDefault();
        const id = $(this).data("id");

        $.ajax({
            url: frontend + "admin/get_talent_flex_our_offering_details",
            type: "POST",
            dataType: "json",
    ...
    $("#TalentFlexOurOfferingTable").on("click", ".edit-btn", function (e) {
        e.preventDefault();
        const id = $(this).data("id");
    ...
    // Delete action
	$("#TalentFlexOurOfferingTable").on("click", ".delete-btn", function (e) {
		e.preventDefault();
		const id = $(this).data("id");
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				$.ajax({
					url: frontend + "admin/delete_talent_flex_our_offering",
					type: "POST",
					data: { id: id },
					dataType: "json",
					success: function (response) {
...
$('#EditTalentFlexOurOfferingForm').submit(function (e) {
    e.preventDefault();
    ...
    $.ajax({
        url: frontend + "admin/update_talent_flex_our_offering", // adjust to your route
        type: "POST",
        data: formData,
        dataType: "json",
        contentType: false,
        ...
        success: function (response) {
            if (response.status === 'success') {
                ...
                // Reset the form
                $('#EditTalentFlexOurOfferingForm')[0].reset();
