$(document).ready(function () {
    $('#PackagingProcurementOurLeadersForm').on('submit', function (e) {
        e.preventDefault();
       // Clear previous error messages
        $('#error_name, #error_designation, #error_image, #error_link').text('');
        var formData = new FormData(this);
        $.ajax({
            url: frontend + "admin/save_packaging_procurement_our_leaders",  // Adjust URL accordingly
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
            ...
            success: function (response) {
                if (response.status === 'success') {
                    ...
                    $('#PackagingProcurementOurLeadersForm')[0].reset();
                     // Reload the DataTable
                    PackagingProcurementOurLeadersTable.ajax.reload(null, false);
...
$(document).ready(function () {
    ...
    PackagingProcurementOurLeadersTable = $('#PackagingProcurementOurLeadersTable').DataTable({
        ajax: {
            url: frontend + "admin/get_packaging_procurement_our_leaders_data",  // Adjust URL accordingly
            type: 'POST',
    ...
    $("#PackagingProcurementOurLeadersTable").on("click", ".view-btn", function (e) {
        e.preventDefault();
        const id = $(this).data("id");

        $.ajax({
            url: frontend + "admin/get_packaging_procurement_our_leaders_details",
            type: "POST",
            dataType: "json",
    ...
    $("#PackagingProcurementOurLeadersTable").on("click", ".edit-btn", function (e) {
        e.preventDefault();
        const id = $(this).data("id");
    ...
    // Delete action
    $("#PackagingProcurementOurLeadersTable").on("click", ".delete-btn", function (e) {
        e.preventDefault();
        const id = $(this).data("id");
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: frontend + "admin/delete_packaging_procurement_our_leaders",
                    type: "POST",
                    data: { id: id },
                    dataType: "json",
                    success: function (response) {
...
$('#EditPackagingProcurementOurLeadersForm').submit(function (e) {
    e.preventDefault();
    ...
    $.ajax({
        url: frontend + "admin/update_packaging_procurement_our_leaders", // adjust to your route
        type: "POST",
        data: formData,
        dataType: "json",
        contentType: false,
        ...
        success: function (response) {
            if (response.status === 'success') {
                ...
                // Reset the form
                $('#EditPackagingProcurementOurLeadersForm')[0].reset();
