$(document).ready(function () {
    $('#PackagingProcurementSuccessStoriesForm').on('submit', function (e) {
        e.preventDefault();
       // Clear previous error messages
        $('#error_title, #error_description, #error_image').text('');
        var formData = new FormData(this);
        $.ajax({
            url: frontend + "admin/save_packaging_procurement_success_stories",  // Adjust URL accordingly
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
            ...
            success: function (response) {
                if (response.status === 'success') {
                    ...
                    $('#PackagingProcurementSuccessStoriesForm')[0].reset();
                     // Reload the DataTable
                    PackagingProcurementSuccessStoriesTable.ajax.reload(null, false);
...
$(document).ready(function () {
    ...
    PackagingProcurementSuccessStoriesTable = $('#PackagingProcurementSuccessStoriesTable').DataTable({
        ajax: {
            url: frontend + "admin/get_packaging_procurement_success_stories_data",  // Adjust URL accordingly
            type: 'POST',
    ...
    $("#PackagingProcurementSuccessStoriesTable").on("click", ".view-btn", function (e) {
        e.preventDefault();
        const id = $(this).data("id");

        $.ajax({
            url: frontend + "admin/get_packaging_procurement_success_stories_details",
            type: "POST",
            dataType: "json",
    ...
    $("#PackagingProcurementSuccessStoriesTable").on("click", ".edit-btn", function (e) {
        e.preventDefault();
        const id = $(this).data("id");
    ...
    // Delete action
	$("#PackagingProcurementSuccessStoriesTable").on("click", ".delete-btn", function (e) {
		e.preventDefault();
		const id = $(this).data("id");
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				$.ajax({
					url: frontend + "admin/delete_packaging_procurement_success_stories",
					type: "POST",
					data: { id: id },
					dataType: "json",
					success: function (response) {
...
$('#EditPackagingProcurementSuccessStoriesForm').submit(function (e) {
    e.preventDefault();
    ...
    $.ajax({
        url: frontend + "admin/update_packaging_procurement_success_stories", // adjust to your route
        type: "POST",
        data: formData,
        dataType: "json",
        contentType: false,
        ...
        success: function (response) {
            if (response.status === 'success') {
                ...
                // Reset the form
                $('#EditPackagingProcurementSuccessStoriesForm')[0].reset();
