$(document).ready(function () {
    $('#WhyPeopleChoosePackforaForm').on('submit', function (e) {
        e.preventDefault();
        ...
        $.ajax({
            url: frontend + "admin/save_why_people_choose_packfora",  // Adjust URL accordingly
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
            ...
            success: function (response) {
                if (response.status === 'success') {
                    ...
                    $('#WhyPeopleChoosePackforaForm')[0].reset();
                     // Reload the DataTable
                    WhyPeopleChoosePackforaTable.ajax.reload(null, false);
...
$(document).ready(function () {
    ...
    WhyPeopleChoosePackforaTable = $('#WhyPeopleChoosePackforaTable').DataTable({
        ajax: {
            url: frontend + "admin/get_why_people_choose_packfora_data",  // Adjust URL accordingly
            type: 'POST',
    ...
    $("#WhyPeopleChoosePackforaTable").on("click", ".view-btn", function (e) {
        e.preventDefault();
        const id = $(this).data("id");

        $.ajax({
            url: frontend + "admin/get_why_people_choose_packfora_details",
            type: "POST",
            dataType: "json",
    ...
    $("#WhyPeopleChoosePackforaTable").on("click", ".edit-btn", function (e) {
        e.preventDefault();
        const id = $(this).data("id");
    ...
    // Delete action
	$("#WhyPeopleChoosePackforaTable").on("click", ".delete-btn", function (e) {
		e.preventDefault();
		const id = $(this).data("id");
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				$.ajax({
					url: frontend + "admin/delete_why_people_choose_packfora",
					type: "POST",
					data: { id: id },
					dataType: "json",
					success: function (response) {
...
$('#EditWhyPeopleChoosePackforaForm').submit(function (e) {
    e.preventDefault();
    ...
    $.ajax({
        url: frontend + "admin/update_why_people_choose_packfora", // adjust to your route
        type: "POST",
        data: formData,
        dataType: "json",
        contentType: false,
        ...
        success: function (response) {
            if (response.status === 'success') {
                ...
                // Reset the form
                $('#EditWhyPeopleChoosePackforaForm')[0].reset();
