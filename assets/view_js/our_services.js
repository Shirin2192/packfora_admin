$(document).ready(function () {
    $('#OurServicesForm').on('submit', function (e) {
        e.preventDefault();
        // Clear previous error messages
        $('#error_link, #error_description, #error_image, #error_title').text('');
        var formData = new FormData(this);
        $.ajax({
            url: frontend + "admin/save_our_services",  // Adjust URL accordingly
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
            ...
            success: function (response) {
                if (response.status === 'success') {
                    ...
                    OurServicesTable.ajax.reload(null, false);
...
$(document).ready(function () {
    ...
    OurServicesTable = $('#OurServicesTable').DataTable({
        ajax: {
            url: frontend + "admin/get_our_services_data",  // Adjust URL accordingly
            type: 'POST',
...
//                         </a>
    ...
    $("#OurServicesTable").on("click", ".view-btn", function (e) {
        e.preventDefault();
        const id = $(this).data("id");

        $.ajax({
            url: frontend + "admin/get_our_services_details",
            type: "POST",
            dataType: "json",
            data: { id: id }, // send id in POST data
            success: function (response) {
                $("#view_title").text(response.data.service_name);
    ...
    $("#OurServicesTable").on("click", ".edit-btn", function (e) {
        e.preventDefault();
        const id = $(this).data("id");
    ...
    // Delete action
	$("#OurServicesTable").on("click", ".delete-btn", function (e) {
		e.preventDefault();
		const id = $(this).data("id");
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				$.ajax({
					url: frontend + "admin/delete_our_services",
					type: "POST",
					data: { id: id },
					dataType: "json",
					success: function (response) {
...
$('#EditOurServicesForm').submit(function (e) {
    e.preventDefault();
    ...
    $.ajax({
        url: frontend + "admin/update_our_services", // adjust to your route
        type: "POST",
        data: formData,
        dataType: "json",
        contentType: false,
