$(document).ready(function () {
    $('#SpecificationManagementOurOfferingForm').on('submit', function (e) {
        e.preventDefault();
       // Clear previous error messages
        $('#error_title, #error_description, #error_image').text('');
        var formData = new FormData(this);
        $.ajax({
            url: frontend + "admin/save_specification_management_our_offering",  // Adjust URL accordingly
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
            ...
            success: function (response) {
                if (response.status === 'success') {
                    ...
                    $('#SpecificationManagementOurOfferingForm')[0].reset();
                     // Reload the DataTable
                    SpecificationManagementOurOfferingTable.ajax.reload(null, false);
...
$(document).ready(function () {
    ...
    SpecificationManagementOurOfferingTable = $('#SpecificationManagementOurOfferingTable').DataTable({
        ajax: {
            url: frontend + "admin/get_specification_management_our_offerings_data",  // Adjust URL accordingly
            type: 'POST',
    ...
    $("#SpecificationManagementOurOfferingTable").on("click", ".view-btn", function (e) {
        e.preventDefault();
        const id = $(this).data("id");

        $.ajax({
            url: frontend + "admin/get_specification_management_our_offering_details",
            type: "POST",
            dataType: "json",
    ...
    $("#SpecificationManagementOurOfferingTable").on("click", ".edit-btn", function (e) {
        e.preventDefault();
        const id = $(this).data("id");
    ...
    // Delete action
	$("#SpecificationManagementOurOfferingTable").on("click", ".delete-btn", function (e) {
		e.preventDefault();
		const id = $(this).data("id");
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				$.ajax({
					url: frontend + "admin/delete_specification_management_our_offering",
					type: "POST",
					data: { id: id },
					dataType: "json",
					success: function (response) {
...
$('#EditSpecificationManagementOurOfferingForm').submit(function (e) {
    e.preventDefault();
    ...
    $.ajax({
        url: frontend + "admin/update_specification_management_our_offering", // adjust to your route
        type: "POST",
        data: formData,
        dataType: "json",
        contentType: false,
        ...
        success: function (response) {
            if (response.status === 'success') {
                ...
                // Reset the form
                $('#EditSpecificationManagementOurOfferingForm')[0].reset();
