$(document).ready(function () {
    $('#SpecificationManagementSuccessStoriesForm').on('submit', function (e) {
        e.preventDefault();
       // Clear previous error messages
        $('#error_title, #error_description, #error_image').text('');
        var formData = new FormData(this);
        $.ajax({
            url: frontend + "admin/save_specification_management_success_stories",  // Adjust URL accordingly
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
            ...
            success: function (response) {
                if (response.status === 'success') {
                    ...
                    $('#SpecificationManagementSuccessStoriesForm')[0].reset();
                     // Reload the DataTable
                    SpecificationManagementSuccessStoriesTable.ajax.reload(null, false);
...
$(document).ready(function () {
    ...
    SpecificationManagementSuccessStoriesTable = $('#SpecificationManagementSuccessStoriesTable').DataTable({
        ajax: {
            url: frontend + "admin/get_specification_management_success_stories_data",  // Adjust URL accordingly
            type: 'POST',
    ...
    $("#SpecificationManagementSuccessStoriesTable").on("click", ".view-btn", function (e) {
        e.preventDefault();
        const id = $(this).data("id");

        $.ajax({
            url: frontend + "admin/get_specification_management_success_stories_details",
            type: "POST",
            dataType: "json",
    ...
    $("#SpecificationManagementSuccessStoriesTable").on("click", ".edit-btn", function (e) {
        e.preventDefault();
        const id = $(this).data("id");
    ...
    // Delete action
	$("#SpecificationManagementSuccessStoriesTable").on("click", ".delete-btn", function (e) {
		e.preventDefault();
		const id = $(this).data("id");
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				$.ajax({
					url: frontend + "admin/delete_specification_management_success_stories",
					type: "POST",
					data: { id: id },
					dataType: "json",
					success: function (response) {
...
$('#EditSpecificationManagementSuccessStoriesForm').submit(function (e) {
    e.preventDefault();
    ...
    $.ajax({
        url: frontend + "admin/update_specification_management_success_stories", // adjust to your route
        type: "POST",
        data: formData,
        dataType: "json",
        contentType: false,
        ...
        success: function (response) {
            if (response.status === 'success') {
                ...
                // Reset the form
                $('#EditSpecificationManagementSuccessStoriesForm')[0].reset();
