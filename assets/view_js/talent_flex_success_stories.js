$(document).ready(function () {
    $('#TalentFlexSuccessStoriesForm').on('submit', function (e) {
        e.preventDefault();
       // Clear previous error messages
        $('#error_title, #error_description, #error_image').text('');
        var formData = new FormData(this);
        $.ajax({
            url: frontend + "admin/save_talent_flex_success_stories",  // Adjust URL accordingly
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
            ...
            success: function (response) {
                if (response.status === 'success') {
                    ...
                    $('#TalentFlexSuccessStoriesForm')[0].reset();
                     // Reload the DataTable
                    TalentFlexSuccessStoriesTable.ajax.reload(null, false);
...
$(document).ready(function () {
    ...
    TalentFlexSuccessStoriesTable = $('#TalentFlexSuccessStoriesTable').DataTable({
        ajax: {
            url: frontend + "admin/get_talent_flex_success_stories_data",  // Adjust URL accordingly
            type: 'POST',
    ...
    $("#TalentFlexSuccessStoriesTable").on("click", ".view-btn", function (e) {
        e.preventDefault();
        const id = $(this).data("id");

        $.ajax({
            url: frontend + "admin/get_talent_flex_success_stories_details",
            type: "POST",
            dataType: "json",
    ...
    $("#TalentFlexSuccessStoriesTable").on("click", ".edit-btn", function (e) {
        e.preventDefault();
        const id = $(this).data("id");
    ...
    // Delete action
	$("#TalentFlexSuccessStoriesTable").on("click", ".delete-btn", function (e) {
		e.preventDefault();
		const id = $(this).data("id");
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				$.ajax({
					url: frontend + "admin/delete_talent_flex_success_stories",
					type: "POST",
					data: { id: id },
					dataType: "json",
					success: function (response) {
...
$('#EditTalentFlexSuccessStoriesForm').submit(function (e) {
    e.preventDefault();
    ...
    $.ajax({
        url: frontend + "admin/update_talent_flex_success_stories", // adjust to your route
        type: "POST",
        data: formData,
        dataType: "json",
        contentType: false,
        ...
        success: function (response) {
            if (response.status === 'success') {
                ...
                // Reset the form
                $('#EditTalentFlexSuccessStoriesForm')[0].reset();
