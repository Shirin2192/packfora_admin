$(document).ready(function () {
    $('#TalentFlexVideoBannerForm').on('submit', function (e) {
        e.preventDefault();
        // Clear previous error messages
        $('#error_video').text('');
        var formData = new FormData(this);
        $.ajax({
            url: frontend + "admin/save_talent_flex_banner_video",  // Adjust URL accordingly
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
            ...
            success: function (response) {
                if (response.status === 'success') {
                    ...
                    $('#TalentFlexVideoBannerForm')[0].reset();
                     // Reload the DataTable
                    TalentFlexVideoBannerTable.ajax.reload(null, false);
...
$(document).ready(function () {
    ...
    TalentFlexVideoBannerTable = $('#TalentFlexVideoBannerTable').DataTable({
        ajax: {
            url: frontend + "admin/get_talent_flex_banner_video_data",  // Adjust URL accordingly
            type: 'POST',
    ...
    $("#TalentFlexVideoBannerTable").on("click", ".view-btn", function (e) {
        e.preventDefault();
        const id = $(this).data("id");

        $.ajax({
            url: frontend + "admin/get_talent_flex_banner_video_details",
            type: "POST",
            dataType: "json",
    ...
    $("#TalentFlexVideoBannerTable").on("click", ".edit-btn", function (e) {
        e.preventDefault();
        const id = $(this).data("id");
    ...
    // Delete action
	$("#TalentFlexVideoBannerTable").on("click", ".delete-btn", function (e) {
		e.preventDefault();
		const id = $(this).data("id");
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				$.ajax({
					url: frontend + "admin/delete_talent_flex_banner_video",
					type: "POST",
					data: { id: id },
					dataType: "json",
					success: function (response) {
...
$('#EditTalentFlexVideoBannerForm').submit(function (e) {
    e.preventDefault();
    ...
    $.ajax({
        url: frontend + "admin/update_talent_flex_banner_video", // adjust to your route
        type: "POST",
        data: formData,
        dataType: "json",
        contentType: false,
        ...
        success: function (response) {
            if (response.status === 'success') {
                ...
                // Reset the form
                $('#EditTalentFlexVideoBannerForm')[0].reset();
