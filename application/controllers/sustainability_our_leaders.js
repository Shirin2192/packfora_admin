$(document).ready(function () {
    $('#SustainabilityOurLeadersForm').on('submit', function (e) {
        e.preventDefault();
       // Clear previous error messages
        $('#error_name, #error_designation, #error_image, #error_link').text('');
        var formData = new FormData(this);
        $.ajax({
            url: frontend + "admin/save_sustainability_our_leaders",  // Adjust URL accordingly
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
            ...
            success: function (response) {
                if (response.status === 'success') {
                    ...
                    $('#SustainabilityOurLeadersForm')[0].reset();
                     // Reload the DataTable
                    SustainabilityOurLeadersTable.ajax.reload(null, false);
...
$(document).ready(function () {
    ...
    SustainabilityOurLeadersTable = $('#SustainabilityOurLeadersTable').DataTable({
        ajax: {
            url: frontend + "admin/get_sustainability_our_leaders_data",  // Adjust URL accordingly
            type: 'POST',
    ...
    $("#SustainabilityOurLeadersTable").on("click", ".view-btn", function (e) {
        e.preventDefault();
        const id = $(this).data("id");

        $.ajax({
            url: frontend + "admin/get_sustainability_our_leaders_details",
            type: "POST",
            dataType: "json",
    ...
    $("#SustainabilityOurLeadersTable").on("click", ".edit-btn", function (e) {
        e.preventDefault();
        const id = $(this).data("id");
    ...
    // Delete action
    $("#SustainabilityOurLeadersTable").on("click", ".delete-btn", function (e) {
        e.preventDefault();
        const id = $(this).data("id");
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: frontend + "admin/delete_sustainability_our_leaders",
                    type: "POST",
                    data: { id: id },
                    dataType: "json",
                    success: function (response) {
...
$('#EditSustainabilityOurLeadersForm').submit(function (e) {
    e.preventDefault();
    ...
    $.ajax({
        url: frontend + "admin/update_sustainability_our_leaders", // adjust to your route
        type: "POST",
        data: formData,
        dataType: "json",
        contentType: false,
        ...
        success: function (response) {
            if (response.status === 'success') {
                ...
                // Reset the form
                $('#EditSustainabilityOurLeadersForm')[0].reset();
